...
const POST = async ({ request }) => {
  try {
    ...
    const host = undefined                          || process.env.SMTP_HOST;
    const port = Number(undefined                          || process.env.SMTP_PORT) || 587;
    const user = undefined                          || process.env.SMTP_USER;
    const pass = undefined                          || process.env.SMTP_PASS;
    const from = undefined                          || process.env.SMTP_FROM || "no-reply@point.ml";
    let emailSent = false;
    let emailError = "";
    if (host && user && pass) {
      try {
        ...
        const htmlContent = `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
            ...
            <div style="background-color: #f9f9f9; padding: 15px; border-radius: 6px; margin-top: 20px; border-left: 4px solid #1a56db;">
              ...
              <p><strong>Date d'enregistrement :</strong> ${(/* @__PURE__ */ new Date()).toLocaleDateString("fr-FR")}</p>
              <p><strong>Date d'expiration :</strong> ${new Date((/* @__PURE__ */ new Date()).setFullYear((/* @__PURE__ */ new Date()).getFullYear() + 1)).toLocaleDateString("fr-FR")}</p>
            </div>
